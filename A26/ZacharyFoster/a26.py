...
class Main:
	...
	def load_config(self):
		...
		self.operating = json_vals['operating']
	...
	def run(self):
		...
		output_filename = self.alpha_sweep["output_file"]
		with open(output_filename, 'w') as outfile:
			...
			if self.run_commands["alpha_sweep"] == True:
				...
				for alpha in alphas:
					...
					CL, Cmle, Cmc4, x_cp, y_cp, _ = self.pannelmethod.run(self.x_geo, self.y_geo)
					print(f"Alpha: {alpha}")
					print(f"CL: {CL[0]}")
					print(f"Cmle: {Cmle[0]}")
					print(f"Cmc4: {Cmc4[0]}")
				...
				if self.run_commands['plot_streamlines'] == True:
					...
					self.setup_vortex_pannel_method(self.operating["alpha[deg]"])
					self.load_flow_field(self.operating["alpha[deg]"])
